...
from swag.util.path import mkdirs
...
def create_contigs_files(picard_ref_seq_dict, contig_interval_size,
                         swag_analysis_dir, swag_inputdata_symlinks):
    """
    ...
    :param swag_analysis_dir: str Relative path to the analysis directory
    :param swag_inputdata_symlinks: Relative paths to all symlinks linked to the original data
    :return: str The filepath to the contigs file
    """
    swag_reference_dir = mkdirs(os.path.join(swag_analysis_dir, SwagStrings.analysis_reference_dir))

    # Parse the given Picard Sequence Dictionary for defined contig names, write out to file
    defined_contigs, contigs_file_fp = extract_contigs_from_dict(picard_ref_seq_dict, swag_reference_dir)

    # Create the contig interval files for each segment
    segment_genomic_regions(picard_ref_seq_dict, contig_interval_size, swag_reference_dir)

    # For every symlink corresponding to input data, do something
    for swag_inputdata_symlink in swag_inputdata_symlinks:
        mk_contigs_file(swag_inputdata_symlink, defined_contigs)

    return contigs_file_fp


def extract_contigs_from_dict(picard_ref_seq_dict, swag_reference_dir):
    """
    Given a Picard Sequence Dictionary, extracts all defined contig names and writes them out 
    to a file in the swag reference directory.
    
    Side effects: Creates a file on disk of the names of all contigs defined in the given 
    Picard Sequence Dictionary.
    
    :param picard_ref_seq_dict: str Path to a Picard Sequence Dictionary (*.dict)
    :param swag_reference_dir: str Parent directory of all contig files
    :return: (list, str) A list of the contigs found in the Picard Sequence Dictionary,
                         filepath to the contigs file
    """
    contigs = list()
    contigs_filepath = os.path.join(swag_reference_dir, SwagStrings.contigs_filename)
...
def segment_genomic_regions(picard_ref_dict, interval_size, swag_reference_dir):
    """
    ...
    case of the last interval), from 1 to the size of the contig. The files are output into the Swag 
    reference directory.
    ...
    contig_segments_unmapped = os.path.join(swag_reference_dir, SwagStrings.contig_unmapped_filename)
    with open(contig_segments_unmapped, 'w') as contigs_unmapped:
        contigs_unmapped.write('no_mapped_reads\n')


def mk_contigs_file(swag_inputdata_symlink, defined_contigs):
    ...
    in the sample directory. Parsl will use this file to know what
    The expected output of splits are. 
    
    NOTE: This function may be depricated in the latest version of
    Swag. Parsl may only need to use the Contigs.txt file
    
    Now this function is being used again since bamtools needs it"""

    bam_dir, bam_prefix = os.path.split(swag_inputdata_symlink)
    bam_prefix = os.path.splitext(bam_prefix)[PREFIX]
    out_dir = os.path.join(os.getcwd(), bam_dir)

    with open(os.path.join(out_dir, SwagStrings.sample_contigs_filename), 'w') as contigs_file:
